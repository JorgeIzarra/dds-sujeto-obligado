...
| DEF-S9-07 | Error al guardar el formulario (RN-04/CA-08) porque el tipo de documento del cliente es CEDULA/PASAPORTE, pero en el frontend (Paso 4) el valor enviado es IDENTIFICACION. | Alta | ✅ Resuelto | Se modificó `existeIdentidadVerificada` en `DocumentoRepository` para aceptar tanto el tipo específico del cliente como el valor genérico IDENTIFICACION. |
| DEF-S9-08 | Vulnerabilidad de Open Redirect (CWE S5146 / tssecurity:S5146) bloqueante en SonarCloud: el middleware HTTPS usaba `req.headers.host` (controlable por atacante) para construir la URL de redirección. | Bloqueante | ✅ Resuelto | Se reemplazó `req.headers.host` por la variable de entorno fija `APP_HOST` y se validó `req.url` con regex para rechazar paths externos. Se ajustaron los tests para reflejar el código `301` y el host fijo. |
